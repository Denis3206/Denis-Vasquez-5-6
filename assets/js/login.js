
const Usuario = document.getElementById("Usuario");
const Contraseña=document.getElementById("Contraseña");
let login =document.getElementById("login");
let inicio_login= document.getElementById("inicio_login");

let  btnLogin=document.getElementById("btn-login");
let insertUser=document.getElementById("User");

const adminLogin=JSON.parse(localStorage.getItem("adminLogin")) || {
    admin:false
}

if (adminLogin.admin){
insertUser.innerHTML=`<p id="user-name">Hola, Administrador</p>`;
inicio_login.innerText="Cerrar";
login.innerHTML=""
login.innerHTML=`<h1>Hola, admin</h1>
<button type="submit" class="login-submit btn btn-primary" id="btn-unlog">Cerrar Sesión</button>
`;
let btnUnlog = document.getElementById("btn-unlog");
btnUnlog.onclick = (e) => {
    e.preventDefault();
    adminLogin.admin = false;
    localStorage.setItem("adminLogin", JSON.stringify(adminLogin));
    window.location.reload();
}

}


/* let prodComprados = document.getElementById("prod-comprados");
let contadorProductos = localStorage.getItem("prod-comprados");

function contadorProd() {
    prodComprados.innerHTML = `${contadorProductos}`;
    prodComprados.style.display = "block";
    if (contadorProductos <= 0 || contadorProductos == undefined) {
        prodComprados.style.display = "none";
    }
}
contadorProd(); */



btnLogin.onclick = (e) => {
    e.preventDefault();
    
    if (Usuario.value == "admin" && Contraseña.value == "123") {
        adminLogin.admin = true;
        console.log(adminLogin.admin)
            // redirecciona a home si el login es exitoso
        window.location.href = "index.html";
    } else {
        Swal.fire({
            icon: 'error',
            title: 'Hubo un problema...',
            text: 'No se pudo ingresar a "' + Usuario.value + '"',
        })
    }
    localStorage.setItem("adminLogin", JSON.stringify(adminLogin));
}
/* btnLogin.onclick=(e)=>{

    e.preventDefault();
    if(Usuario.value !== "admin" && Contraseña.value !=="123" ){
        
    alert("No es administrador")
    
    }
    else{
        window.location.href="index.html"
        alert("Hola, Admin")
        
    }
}; */

   
/* function guardarAlmacenamientoLocal(llave, valor_a_guardar){
    localStorage.setItem(llave,JSON.stringify(valor_a_guardar))
}
function obtenerAlmacenamientoLocal(llave){
    const datos =JSON.parse(localStorage.getItem(llave))
    return datos
}

let prod=obtenerAlmacenamientoLocal('prod')|| [];
let mensaje=docuemnt.getElementById("mensaje")
const nombre=document.getElementById("nombre")
const precio =document.getElementById("precio")

document.getElementById("añadir").addEventListener("click", function(e){
    e.preventDefault();
    let nombre =nombre.value
    let precio=precio.value

    let van=true

    if(nombre=="" || precio==""){
        mensaje.classList.add("llenarCampos")
        setTimeout(()=>{mensaje.classList.remove("llenarCampos")}, 2500)
        van=false

    }
    else{
        for(i=0; i<prod.lenght;i++){
            if(prod[i].nombre==nombre){
                mensaje.classList.add("repetido")
                setTimeout(()=>{mensaje.classList.remove("repetido")}, 2500)
                van=false

            }
        }
    }
    if(van==true){

    
    prod.push({
        nombre: nombre,
        precio: precio,
        img:"https://dummyimage.com/600x400/000/fff"
    })
    mensaje.classList.add("realizado")
    setTimeout(()=>{mensaje.classList.remove("realizado")
    window.location.reload()
}, 1500)
                
    }

    guardarAlmacenamientoLocal('prod',prod);
})
 */
/* const adminLogin=JSON.parse(localStorage.getItem("adminLogin")) || {
    admin:false
}
if (adminLogin.admin){
    inserUser.innerHMTL = `
    <p id="user-name">Hola, Administrador</p>
    `
    inicio_login.innerText="Cerrar Sesión";
    login.innerHTML=""
    login.innerHTML=`
    <h1>Hola, admin</h1>
    <button type="submit" class="login-submit btn btn-primary" id="btn-unlog">Cerrar Sesión</button>
    `
    let btnUnlog=document.getElementById("btn-unlog");
    btnUnlog.onclick=(e)=>{
        e.preventDefault();
        adminLogin.admin=false;
        localStorage.setItem("adminLogin",JSON.stringify(adminLogin));
        window.location.reload();
    }
}

btnLogin.onclick = (e) => {
    e.preventDefault();
    const user = document.getElementById("Usuario").value;
    const pass = document.getElementById("Contraseña").value;
    if (user == "admin" && pass == "123") {
        adminLogin.admin = true;
        console.log(adminLogin.admin)
            // redirecciona a home si el login es exitoso
        window.location.href="index.html";
        mostrar();
    } else {
        Swal.fire({
            icon: 'error',
            title: 'Hubo un problema...',
            text: 'No se pudo ingresar a "' + user + '"',
        })
    }
    localStorage.setItem("adminLogin", JSON.stringify(adminLogin));
} */
